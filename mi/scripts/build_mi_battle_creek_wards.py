#!/usr/bin/env python3
"""Battle Creek's five commission wards — and a name field this build refuses.

The boundary is the city's own: `Wards_BC` on the City of Battle Creek's ArcGIS
org (412 items, including its parcel map and its own StoryMaps), item snippet
"This layer contains the boundaries of all voting wards in Battle Creek along
with associated commissioner names". licenseInfo is empty and the item is shared
public — the Detroit/Warren/Flint case, not Lansing's CC BY-NC.

THE LAYER CARRIES A COMMISSIONER NAME PER WARD AND THIS BUILD SHIPS NONE OF THEM.
That is the same refusal Michigan's own county-commissioner layer earned: it too
carries a name per district, and it lists election winners rather than whoever
holds the seat now. Here the evidence is on the records themselves — four of the
...
measures 97.608% against the same fabric — below this project's 0.99 floor, with
the disagreement spread across a dozen ward pairs rather than sitting on one
edge, which is two different plans and not digitisation noise. Pontiac publishes
precinct layers rather than ward layers (45.969%). Neither ships, and the floor
was not moved for either; both are recorded.
"""
...
CITY_ORG = "https://services6.arcgis.com/cuKwt0IKP5B84jop/arcgis/rest/services"
SERVICE_ROOT = CITY_ORG + "/Wards_BC/FeatureServer"
EXPECT_LAYER_NAME = "Wards_BC"   # the root is read; this asserts what we got

PRECINCTS = ("https://services3.arcgis.com/dxRQUfTDNtfqZ301/arcgis/rest/"
             "services/2026_Voting_Precincts/FeatureServer/0")
PRECINCT_WHERE = "Jurisdiction_Name = 'Battle Creek'"
...
PLACE_GEOID = "2605920"          # Battle Creek city, MI — NAME asserted at run time.
                                 # The first draft guessed 2606020, which is BAY CITY; the
...
EXPECT_PRECINCTS = {"1": 2, "2": 2, "3": 3, "4": 3, "5": 3}
MIN_PRECINCT_AGREEMENT = 0.99    # measured 0.99450 by this build's own sample
# A FETCH SANITY CHECK, NOT A QUALITY GATE, AND SIZED TO THIS CITY. The sibling
# builders carry 2000 here, which is a Grand-Rapids-sized number: Battle Creek's
# envelope holds 1,879 Census 2020 blocks, so the inherited constant refused a
# correct fetch. What this guards is TIGERweb silently truncating or moving —
...
# not a number copied from a larger city. Measured 1,879; floored at 1,500.
...
def fail(msg):
    print("build-mi-battle-creek-wards: FAIL — %s" % msg, file=sys.stderr)
    sys.exit(1)
...
def ward_of(props):
    ...
    """
    for k in ("WARDNUM", "LONGNAME", "DISTRICT", "SHORTNAME", "WARD", "Ward", "ward"):
        v = props.get(k)
        if v in (None, ""):
            continue
...
def fetch_blocks(box):
    ...
    if len(rows) < MIN_BLOCKS:
        raise RuntimeError("only %d usable blocks over Battle Creek's envelope, floor %d — "
...
def assert_no_stated_terms():
    """Battle Creek ships because its item states NO terms — so notice if that changes.
    ...
    item with no stated restriction. If Battle Creek ever adds a licence, that is news
    ...
    if stated:
        fail("the city now STATES TERMS on this item (%r). Battle Creek shipped because it "
...
def ward_layer_url():
    ...
    Battle Creek's ward layer is at index 0, but the index is READ rather than assumed:
...
def main():
    ...
    if args.check:
        ...
        if problems:
            fail("; ".join(problems))
        print("build-mi-battle-creek-wards: OK — 5 wards shipped")
        return
    ...
    if not place or "battle creek" not in str(place[0]["properties"].get("NAME", "")).lower():
        fail("place GEOID %s is not Battle Creek (got %r) — the Census place id is wrong"
    ...
    tmp = os.path.join(APP_DATA_DIR, ".battle-creek-wards-src.json")
    ...
    if problems:
        fail("; ".join(problems))
    print("build-mi-battle-creek-wards: wrote %s (5 wards, %d bytes)"
