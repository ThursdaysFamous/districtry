...
# Floors. Tranche 1 measured 6 counties and 76 seats on 2026-09-13; these
# tolerate any two counties going dark and refuse a collapse. Raise them when
# a tranche lands, never lower one to get past a failure.
MIN_COUNTIES = 4
MIN_DISTRICTS = 45
...
def main():
    ...
    for fips in sorted(cache.get("counties", {})):
        ...
        if set(keyed) != want:
            skipped.append((county, "districts %s, geometry draws %s"
                            % (sorted(keyed, key=int), sorted(want, key=int))))
            continue
