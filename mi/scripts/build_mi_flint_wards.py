#!/usr/bin/env python3
"""Flint's nine council wards — and the sharpest naming trap in the fleet.

Flint elects nine council members, one per ward. The plan in force is the one
adopted effective 2 August 2022. THE SERVICE NAMES ARE ALL BUT INVERTED, and a
builder keyed on a name picks exactly the wrong layer:

  Wards22    <- SHIPS. AGO item titled "Wards23", snippet "Boundaries of the
               City of Flint Wards effective August 2, 2022". 9 features.
               99.784% against the state's current precinct fabric.
  Wards2022  <- the 2012 plan. Its AGO item is honestly titled "City of Flint
               - Old Wards", snippet "Ward Boundaries for City of Flint 2012",
               and it measures 76.886%. The SERVICE name is the newer-looking
               one; the ITEM name is the truthful one.
  Wards_Outline <- also the old plan, 76.955%, and published by the CITY's own
               account (kanpalagan_Flint) with the snippet "These are the nine
               wards in Flint." Being the city's own account does not make a
               layer current.

So three layers all carry nine features called "Ward 1".."Ward 9", two of them
describe a plan a decade out of date, and the one that is right is reached
through an item whose title says 23 and whose service says 22. NOTHING HERE IS
DECIDED BY A NAME. The currency gate below is what picks the layer, and it
separates them by twenty-three points.

TERMS. The item's licenseInfo is empty and it is shared public — the Detroit
and Warren case, not Lansing's CC BY-NC. `assert_no_stated_terms()` re-reads
that before every build and refuses if a licence has appeared, so geometry is
never republished under terms nobody has read.

CURRENCY. The state's own 2026 precinct fabric assigns Flint's 29 precincts
3/4/3/2/3/3/3/3/5 across wards 1-9. Dissolved by that column, the city's nine
polygons agree with the state on 99.784% of sampled points. That agreement and
...
CITY_ORG = "https://services2.arcgis.com/5ckbIY7K9TUKoseK/arcgis/rest/services"
# NOT Wards2022 (the 2012 plan) and NOT Wards_Outline (also 2012). See the docstring.
SERVICE_ROOT = CITY_ORG + "/Wards22/FeatureServer"
EXPECT_LAYER_NAME = "Wards22"   # the root is read; this asserts what we got

PRECINCTS = ("https://services3.arcgis.com/dxRQUfTDNtfqZ301/arcgis/rest/"
             "services/2026_Voting_Precincts/FeatureServer/0")
PRECINCT_WHERE = "Jurisdiction_Name = 'Flint'"
...
PLACE_GEOID = "2629000"          # Flint city, MI — NAME asserted at run time

OUT_FILE = "mi-flint-wards.json"
SIMPLIFY = "50%"                 # nine features; the fleet tolerance, verified below
PRECISION = "0.000001"

EXPECT_FEATURES = 9
EXPECT_WARDS = ("1", "2", "3", "4", "5", "6", "7", "8", "9")
EXPECT_PRECINCTS = {"1": 3, "2": 4, "3": 3, "4": 2, "5": 3,
                    "6": 3, "7": 3, "8": 3, "9": 5}
MIN_PRECINCT_AGREEMENT = 0.99    # measured 0.99784; the two OLD plans score 0.769
...
def fail(msg):
    print("build-mi-flint-wards: FAIL — %s" % msg, file=sys.stderr)
    sys.exit(1)
...
def fetch_blocks(box):
    ...
    for f in d.get("features", []):
        ...
        try:
            ...
            continue
    if len(rows) < 2000:
        raise RuntimeError("only %d usable blocks over Flint's envelope — expected "
...
def assert_no_stated_terms():
    """Flint ships because its item states NO terms — so notice if that changes.
    ...
    item with no stated restriction. If Flint ever adds a licence, that is news
    ...
    if stated:
        fail("the city now STATES TERMS on this item (%r). Flint shipped because it "
...
def ward_layer_url():
    ...
    Flint's ward layer is at index 0, but the index is READ rather than assumed:
    Warren's sits at 32, and a hardcoded index returns HTTP 200 with no
...
def main():
    ...
    if args.check:
        ...
        if problems:
            fail("; ".join(problems))
        print("build-mi-flint-wards: OK — 9 wards shipped")
        return
    ...
    wards = esri(service, {"where": "1=1", "outFields": "LONGNAME,SHORTNAME,DISTRICT"})
    ...
    if not place or "flint" not in str(place[0]["properties"].get("NAME", "")).lower():
        fail("place GEOID %s is not Flint (got %r) — the Census place id is wrong"
    ...
    tmp = os.path.join(APP_DATA_DIR, ".flint-wards-src.json")
    ...
    if problems:
        fail("; ".join(problems))
    print("build-mi-flint-wards: wrote %s (9 wards, %d bytes)"
