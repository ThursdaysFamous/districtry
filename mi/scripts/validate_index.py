...
ROSTER_FILES = {
    ...
    "mi-commissioner-members.json": 4,  # Which commissioner holds each county commissioner district, keyed by 3-digit county FIPS — the same CountyFIPS mi-commissioner-districts.json carries. Built county by county from each county's own board page by mi/scripts/mi_commissioner_scraper.py + build_mi_commissioner_roster.py, refreshed weekly by update-mi-commissioner-roster.yml. IT ARRIVES IN TRANCHES and is not meant to reach 83: tranche 1 is 6 counties and 76 of the 619 districts (2026-09-13). min_keys is a floor that tolerates any two of the six going dark and refuses a collapse — raise it when a tranche lands, never lower it to get past a failure.
}
