...
ROSTER_FILES = {
    ...
    "mi-commissioner-members.json": 24,  # Which commissioner holds each county commissioner district, keyed by 3-digit county FIPS — the same CountyFIPS mi-commissioner-districts.json carries. Built county by county from each county's own board page by mi/scripts/mi_commissioner_scraper.py + build_mi_commissioner_roster.py, refreshed weekly by update-mi-commissioner-roster.yml. IT ARRIVES IN TRANCHES and is not meant to reach 83: 26 counties and 229 of the 619 districts through tranche 5 (2026-09-19), which is 55.1% of Michigan's 10,077,331 people — Census 2020 on both sides of that fraction, because the district layer's own Population column sums to 10,007,596 and reads 0 for all seven of Cheboygan's districts. A county record may be SHORT, and `unnamedDistricts` names which district while `unnamedWhy` carries a per-district reason. THREE ARE SHORT AND FOR THREE DIFFERENT REASONS, each stated in its own words on the card: Monroe's own directory row for District 2 names no commissioner; Ionia's board page calls District 3 vacant; and Lenawee District 5's row names Jim Daly where the same county's News Flash of 10 September 2026 announces his death, so two county surfaces disagree and nothing here names that seat. All three state which district on the card and on the county page rather than dropping it. min_keys is a floor that tolerates any two of the twenty-six going dark and refuses a collapse — raise it when a tranche lands, never lower it to get past a failure.
}
