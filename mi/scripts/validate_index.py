...
GEOMETRY_FILES = {
    ...
    "mi-flint-wards.json": (9, 9),  # Flint's nine council wards, effective 2 August 2022, built by mi/scripts/build_mi_flint_wards.py. THE SERVICE NAMES ARE ALL BUT INVERTED and nothing here is decided by a name: the plan in force is service `Wards22` (whose AGO item is titled "Wards23"), while `Wards2022` is the 2012 plan (its item honestly titled "City of Flint - Old Wards") and `Wards_Outline`, published by the city's own account, is also 2012. All three carry nine features called Ward 1..Ward 9; the currency gate separates them by twenty-three points — 99.525% for the shipped layer against 76.9% for both old ones. The item states no terms and is shared public, and the builder refuses to rebuild if that changes. The nine wards are the most evenly drawn in the fleet: -0.30% to +0.39% on Census 2020, 81,242 against the Census place's 81,252 across ten edge blocks.
    "mi-battle-creek-wards.json": (5, 5),  # Battle Creek's five commission wards, from the City of Battle Creek's own ArcGIS org, built by mi/scripts/build_mi_battle_creek_wards.py. The item states no terms and is shared public (the Detroit/Warren/Flint case). Currency measured against the state's own 2026 precinct WARD column (13 precincts, 2/2/3/3/3) at 99.450%; balance -4.09% to +4.18% on Census 2020, 52,706 against the Census place's 52,721 across 12 edge blocks. THE LAYER CARRIES A COMMISSIONER NAME PER WARD AND NONE OF THEM SHIP: four of five records were last edited in March 2023 and Michigan cities elect in odd Novembers, so a name field with no publication date is not a roster. The names on the card come from the city's own commission page instead — an earlier version of this note said that page answered 403, which was true only of a page id that had been guessed.
    "mi-rochester-hills-wards.json": (4, 4),  # Rochester Hills's four city council districts, from the city's OWN ArcGIS Server (gis.rochesterhills.org), built by mi/scripts/build_mi_rochester_hills_wards.py. The AGO item is shared public with an empty licenseInfo (the Detroit/Warren/Flint/Battle Creek case). THE RECORD SAID NO SERVICE EXISTED: the 23-city sweep reported none, and the city runs both its own AGOL org and its own server — a catalogue that does not list it is not a publisher that lacks it. Currency measured against the state's own 2026 precinct WARD column (21 precincts, 6/5/5/5) at 99.725%; balance -2.72% to +3.36% on Census 2020, 76,138 against the Census place's 76,300. THAT -162 IS ONE BLOCK 26 m INSIDE THE CITY LINE, which is why the builder's population ceiling is 0.22% here against the fleet's 0.2% — raised for this city alone on a measurement, with a new and strictly tightening 100 m edge test added beside it so the raise cannot hide a hole. NO OFFICEHOLDER SHIPS: the city's council page is maintained and its robots.txt disallows general crawlers, so the card names the district and links the city.
    "alger-county-outline.json": (1, 1),  # Alger County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Alger County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "allegan-county-outline.json": (1, 1),  # Allegan County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Allegan County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "antrim-county-outline.json": (1, 1),  # Antrim County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Antrim County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "baraga-county-outline.json": (1, 1),  # Baraga County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Baraga County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "bay-county-outline.json": (1, 1),  # Bay County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Bay County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "benzie-county-outline.json": (1, 1),  # Benzie County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Benzie County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "branch-county-outline.json": (1, 1),  # Branch County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Branch County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "charlevoix-county-outline.json": (1, 1),  # Charlevoix County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Charlevoix County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "crawford-county-outline.json": (1, 1),  # Crawford County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Crawford County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "genesee-county-outline.json": (1, 1),  # Genesee County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Genesee County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "gladwin-county-outline.json": (1, 1),  # Gladwin County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Gladwin County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "gogebic-county-outline.json": (1, 1),  # Gogebic County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Gogebic County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "gratiot-county-outline.json": (1, 1),  # Gratiot County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Gratiot County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "huron-county-outline.json": (1, 1),  # Huron County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Huron County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "ingham-county-outline.json": (1, 1),  # Ingham County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Ingham County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "iosco-county-outline.json": (1, 1),  # Iosco County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Iosco County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "iron-county-outline.json": (1, 1),  # Iron County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Iron County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "keweenaw-county-outline.json": (1, 1),  # Keweenaw County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Keweenaw County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "livingston-county-outline.json": (1, 1),  # Livingston County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Livingston County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "manistee-county-outline.json": (1, 1),  # Manistee County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Manistee County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "marquette-county-outline.json": (1, 1),  # Marquette County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Marquette County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "mason-county-outline.json": (1, 1),  # Mason County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Mason County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "mecosta-county-outline.json": (1, 1),  # Mecosta County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Mecosta County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "missaukee-county-outline.json": (1, 1),  # Missaukee County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Missaukee County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "montmorency-county-outline.json": (1, 1),  # Montmorency County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Montmorency County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "newaygo-county-outline.json": (1, 1),  # Newaygo County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Newaygo County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "oakland-county-outline.json": (1, 1),  # Oakland County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Oakland County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "ogemaw-county-outline.json": (1, 1),  # Ogemaw County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Ogemaw County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "ottawa-county-outline.json": (1, 1),  # Ottawa County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Ottawa County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "shiawassee-county-outline.json": (1, 1),  # Shiawassee County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Shiawassee County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "st-joseph-county-outline.json": (1, 1),  # St. Joseph County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in St. Joseph County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "tuscola-county-outline.json": (1, 1),  # Tuscola County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Tuscola County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "van-buren-county-outline.json": (1, 1),  # Van Buren County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Van Buren County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "washtenaw-county-outline.json": (1, 1),  # Washtenaw County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Washtenaw County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
    "wexford-county-outline.json": (1, 1),  # Wexford County gap-location geometry — NOT a dispatched county and not a coverage test. No layer answers here; this ships so the Data gaps panel can tell a reader standing in Wexford County which recorded gap affects them. Without it `appliesHere` 404s while the panel's `mappable` flag is true, and the lede claims "Nothing recorded is missing where you clicked" inside a county whose board this app cannot answer. Built from TIGERweb's water-inclusive county fabric by mi/scripts/build_mi_gap_outlines.py at the same 25 m tolerance the coverage wash uses, so the two can never disagree about a boundary; verified by the Census's own internal point for every unserved county landing inside its own outline and outside all the others. The county list is DERIVED (all 83 minus those mi-commissioner-members.json covers), so a roster tranche retires this file on its own.
}
...
ROSTER_FILES = {
    ...
    "mi-battle-creek-commission-members.json": 5,  # Battle Creek's City Commission — all nine seats: one commissioner per ward for five wards, three at-large commissioners and the Mayor, the split the city states in its own prose on the page the roster is read from. Built by mi/scripts/build_mi_battle_creek_commission.py from mi_battle_creek_commission_scraper.py's cache. Six top-level keys: wards, citywide, office (City Hall's address, the city switchboard and the shared contact form, all hoisted to the body), seats, seatsPerWard, sourceUrl.
    "mi-commissioner-returns.json": 33,  # Commissioner NAMES for the 35 counties mi-commissioner-members.json does NOT reach, from the state district layer's own certified-returns column, built by mi/scripts/build_mi_returns_roster.py. THIS IS A DIFFERENT KIND OF ROSTER FROM THE FILE ABOVE AND EVERY RECORD SAYS SO: these are the winners the State of Michigan certified in the general election of 5 November 2024, not a list any county keeps current, so each record carries its own `lede` naming that election and the builder REFUSES TO WRITE a record without one. mi-commissioner-districts.json stays name-free and its BANNED_FIELDS guard is untouched — the 2026-09-03 decision to strip those columns off the BOUNDARY file stands. How stale the column is, measured 2026-09-22 on the 366 seats where the scraped roster gives a comparand: 234 identical, 104 the same surname in a different form (Dave/David, middle initials, diacritics, suffixes) and 28 a different surname, which a hand reading splits 18 naming a different person (4.9% of the 366), 9 misspelling the same person and 1 a missing space. Five of the 35 — Washtenaw, Bay, Manistee, Newaygo, Van Buren — do publish their own roster and only the parsing is hard, so each carries a `betterSource` note saying a scraper remains the better answer. min_keys is a floor that tolerates two counties leaving the cohort as they ship a real roster; it FALLS as counties move to the scraped file, which is the one direction it may move without a source change.
    "mi-commissioner-members.json": 46,  # Which commissioner holds each county commissioner district, keyed by 3-digit county FIPS — the same CountyFIPS mi-commissioner-districts.json carries. Built county by county from each county's own board page by mi/scripts/mi_commissioner_scraper.py + build_mi_commissioner_roster.py, refreshed weekly by update-mi-commissioner-roster.yml. IT ARRIVES IN TRANCHES and is not meant to reach 83: 48 counties and 366 of the 619 districts through tranche 7 (2026-09-19), which is 61.4% of Michigan's 10,077,331 people — Census 2020 on both sides of that fraction, because the district layer's own Population column sums to 10,007,596 and reads 0 for all seven of Cheboygan's districts. A county record may be SHORT, and `unnamedDistricts` names which district while `unnamedWhy` carries a per-district reason. FOUR ARE SHORT AND FOR THREE DIFFERENT REASONS, each stated in its own words on the card: Monroe's own directory row for District 2 names no commissioner; Ionia's and Clinton's board pages call District 3 and District 4 vacant, Clinton carrying its own Notice of Vacancy for that seat on the same page; and Lenawee District 5's row names Jim Daly where the same county's News Flash of 10 September 2026 announces his death, so two county surfaces disagree and nothing here names that seat. All four state which district on the card and on the county page rather than dropping it. min_keys is a floor that tolerates any two of the forty-eight going dark and refuses a collapse — raise it when a tranche lands, never lower it to get past a failure.
}
...
DYNAMIC_REFERENCE = frozenset({
    "alger-county-outline.json",
    "allegan-county-outline.json",
    "antrim-county-outline.json",
    "baraga-county-outline.json",
    "bay-county-outline.json",
    "benzie-county-outline.json",
    "branch-county-outline.json",
    "charlevoix-county-outline.json",
    "crawford-county-outline.json",
    "genesee-county-outline.json",
    "gladwin-county-outline.json",
    "gogebic-county-outline.json",
    "gratiot-county-outline.json",
    "huron-county-outline.json",
    "ingham-county-outline.json",
    "iosco-county-outline.json",
    "iron-county-outline.json",
    "keweenaw-county-outline.json",
    "livingston-county-outline.json",
    "manistee-county-outline.json",
    "marquette-county-outline.json",
    "mason-county-outline.json",
    "mecosta-county-outline.json",
    "missaukee-county-outline.json",
    "montmorency-county-outline.json",
    "newaygo-county-outline.json",
    "oakland-county-outline.json",
    "ogemaw-county-outline.json",
    "ottawa-county-outline.json",
    "shiawassee-county-outline.json",
    "st-joseph-county-outline.json",
    "tuscola-county-outline.json",
    "van-buren-county-outline.json",
    "washtenaw-county-outline.json",
    "wexford-county-outline.json",
})
