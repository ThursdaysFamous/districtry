...
ROSTER_FILES = {
    ...
    "mi-commissioner-members.json": 9,  # Which commissioner holds each county commissioner district, keyed by 3-digit county FIPS — the same CountyFIPS mi-commissioner-districts.json carries. Built county by county from each county's own board page by mi/scripts/mi_commissioner_scraper.py + build_mi_commissioner_roster.py, refreshed weekly by update-mi-commissioner-roster.yml. IT ARRIVES IN TRANCHES and is not meant to reach 83: 11 counties and 119 of the 619 districts through tranche 2 (2026-09-15). A county record may be SHORT — Monroe carries unnamedDistricts ["2"] beside 8 names, because its own directory row for District 2 names no commissioner — and the card and the county page both state which district rather than dropping it. min_keys is a floor that tolerates any two of the eleven going dark and refuses a collapse — raise it when a tranche lands, never lower it to get past a failure.
}
