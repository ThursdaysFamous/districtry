...
ROSTER_FILES = {
    ...
    "mi-commissioner-members.json": 14,  # Which commissioner holds each county commissioner district, keyed by 3-digit county FIPS — the same CountyFIPS mi-commissioner-districts.json carries. Built county by county from each county's own board page by mi/scripts/mi_commissioner_scraper.py + build_mi_commissioner_roster.py, refreshed weekly by update-mi-commissioner-roster.yml. IT ARRIVES IN TRANCHES and is not meant to reach 83: 16 counties and 165 of the 619 districts through tranche 3 (2026-09-15), which is 51.8% of Michigan's people. A county record may be SHORT, and `unnamedDistricts` names which district while `unnamedWhy` carries a per-district reason where the shortfalls differ. TWO ARE SHORT FOR DIFFERENT REASONS: Monroe's own directory row for District 2 names no commissioner, and Lenawee District 5's row names Jim Daly where the same county's News Flash of 10 September 2026 announces his death, so two county surfaces disagree and nothing here names that seat. Both state which district on the card and on the county page rather than dropping it. min_keys is a floor that tolerates any two of the sixteen going dark and refuses a collapse — raise it when a tranche lands, never lower it to get past a failure.
}
