...
GEOMETRY_FILES = {
    ...
    "wi-senate-districts.json": (34, 34),  # The 33 State Senate districts plus TIGERweb's ZZ water pseudo-district, pre-built with wi-assembly-districts.json in ONE mapshaper run by wi/scripts/build_legislative_boundaries.py, so the shared edges are one arc. Three gates: 2,000-point agreement, exact Senate/Assembly nesting, and a 20 m fidelity ceiling against the source.
    "wi-assembly-districts.json": (100, 100),  # The 99 State Assembly districts plus TIGERweb's ZZ water pseudo-district, pre-built with wi-senate-districts.json in ONE mapshaper run by wi/scripts/build_legislative_boundaries.py, so the shared edges are one arc. Three gates: 2,000-point agreement, exact Senate/Assembly nesting, and a 20 m fidelity ceiling against the source.
...
ROSTER_FILES = {
    ...
    "wi-alderpersons.json": 31,  # The alderperson roster for the municipalities with a verified, witnessed route — 292 alderpersons across 265 districts in 31 municipalities holding 293 seats, measured 2026-09-25. THE MUNICIPALITIES ARE NOT ENUMERATED HERE ANY MORE: this note listed them until that date and the list went stale twice, saying 24 municipalities and 240 seats while the file held 30 and 285, and then naming 28 of the 30, so FLOORS in wi/scripts/build_wi_alderperson_roster.py is the list and this is the measurement. Keyed by the municipality's statewide COUSUBFP and zero-padded district id, the geometry file's exact key pair. `members[district]` is a LIST, always: fifteen of the twenty-two municipalities the 2026-09-06 sweep matched seat MORE THAN ONE alderperson per district and Wautoma seats one, three and two, so the shape holds an arbitrary count. `vacantSeats` counts, per district, the seats a city itself lists as vacant INSIDE a district that names somebody — Oconomowoc seats two per district and its own directory prints `Vacanct` for one of District 1's — which is a different claim from `vacantDistricts`, where a city names nobody for a whole district; a district where a city seats two, names one and says nothing about the other is neither, and is not built. Built by wi/scripts/build_wi_alderperson_roster.py with per-municipality, per-field floors and a geometry cross-gate; a municipality whose page could not be read keeps its last shipped rows and carries a carriedFrom date the card renders, so nothing rides stale silently. Refreshed weekly by update-wi-alderperson-roster.yml.
    ...
    "wi-county-facts.json": 72,  # Each of the 72 counties' SEAT, keyed by county GEOID — the same Blue Book section 190 scrape and the same permission. It is county-keyed because it is a column of the counties table and therefore the county's own fact: matched onto the city/village fabric instead it degrades to 67, because three seats are not incorporated municipalities at all (Meenon, Florence, Keshena) and two are ambiguous — Douglas and Waukesha each have a City AND a Village of the seat's name, and the column prints a name with no class. The year each county was created sits beside the seat in that table and is NOT taken. An OPERATOR build, biennial.
    ...
    "wi-municipal-executives.json": 19,  # Milwaukee County's 19 municipal executives (mayors and village presidents) keyed by place GEOID, from the county GIS & Land Information layer — the only source in Wisconsin that names them as data. The layer's own dataLastEditDate is 2024-07-30 and these offices are elected every April of even years, so a NAME ships only where the municipality's OWN page witnessed it (wi_municipal_executive_scraper.py + build_wi_municipal_executives.py); 9 witnessed and 10 withheld with the reason at first build; 8 and 11 from 2026-09-18, when robots.txt was read before the first fetch of every host and four municipalities turned out to ask automated clients not to read them (St. Francis and Cudahy publish `User-agent: * / Disallow: /`; Milwaukee and Wauwatosa answer 403 on robots.txt, which a municipal website takes strictly). St. Francis had been witnessed until then. Refreshed weekly, and a name is NOT deleted because one fetch failed: where the page is unreachable the last witnessed name is carried with a `carriedFrom` date the card states, never past 60 days, an April spring election, a refusal, a dead link, a page that read without witnessing, or a layer that has stopped naming that person.
    "wi-municipal-facts.json": 600,  # Incorporation year and county (or counties) for 607 of Wisconsin's 608 cities and villages, keyed by place GEOID — from the Wisconsin Blue Book 2025–26 section 190, used by written permission from the Legislative Reference Bureau (LICENSE-DATA.md §4). wi/scripts/wi_bluebook_municipal_scraper.py + build_wi_municipal_facts.py. The parse is gated on the source's OWN class headings, which count the cities for it (1 + 16 + 34 + 139 = 190). 57 municipalities name MORE THAN ONE county and the card names them all, where the Elections Commission's own field says only "Multiple Counties" for 58 of them. The one municipality absent is the VILLAGE OF GREENLEAF, which incorporated after the table's January 1, 2024 snapshot; its card says that rather than rendering a blank. Population, area, rank and equalized value sit in the same tables and are NOT taken — the permission covers three facts. An OPERATOR build on the Blue Book's own biennial cadence, not a weekly one.
