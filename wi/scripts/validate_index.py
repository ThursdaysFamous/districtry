...
ROSTER_FILES = {
    ...
    "wi-alderpersons.json": 24,  # The alderperson roster for the 24 municipalities with a verified, witnessed route — the six big cities of 2026-08-26 (Milwaukee, Madison, Green Bay, Kenosha, Racine, Waukesha) plus the twelve of 2026-09-05 (Stevens Point, Menomonie, Manitowoc, Sheboygan, Superior, Portage, Viroqua, Menasha, Howard, Tomah, Eau Claire, Appleton) and the five of 2026-09-05 evening (New Berlin, Sturgeon Bay, Altoona, Eagle River, Germantown) and New Lisbon (2026-09-06) — 240 districts and 240 alderpersons measured 2026-09-24, where this note read 239 — keyed by the municipality's statewide COUSUBFP and zero-padded district id, the geometry file's exact key pair. `members[district]` is a LIST, always: fifteen of the twenty-two municipalities the 2026-09-06 sweep matched seat MORE THAN ONE alderperson per district and Wautoma seats one, three and two, so the shape holds an arbitrary count and every municipality shipped today has one-element lists. Built by wi/scripts/build_wi_alderperson_roster.py with per-municipality, per-field floors and a geometry cross-gate; a municipality whose page could not be read keeps its last shipped rows and carries a carriedFrom date the card renders, so nothing rides stale silently. Refreshed weekly by update-wi-alderperson-roster.yml.
