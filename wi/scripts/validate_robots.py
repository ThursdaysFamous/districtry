...
# THE ROBOTS PARSER MOVED TO ONE COPY at the repo root (2026-09-12), because two
# more consumers appeared: the fleet user-agent probe and a scraper that has to
# ask before it fetches. star_disallows(), permitted() and resolve_template()
# carry the record of what a literal startswith() got wrong, and a second copy
# of that record is how it drifts. APPENDED rather than inserted, so this
# instance's own modules still win any name collision — inserting the root
# scripts/ first is how a bare `import validate_sources` once resolved to
# Illinois's and made this sweep report Kane and Coles hosts as Wisconsin's.
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(SCRIPT_DIR)),
                             "scripts"))
from robots_rules import (permitted, resolve_template,  # noqa: E402
                          star_disallows)
