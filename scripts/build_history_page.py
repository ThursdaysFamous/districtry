...
from measured_metric import (METRIC_RE, PERSON_WORDS, people_in,  # noqa: F401
                             measure_metric as _measure_metric)
...
def measure_metric(inst, spec):
    """This module's own arguments, the shared evaluator's answer.

    The grammar and the measuring live in scripts/measured_metric.py so this
    file and validate_gap_counts.py cannot drift about what `keys` means. The
    wrapper exists so every call site here is unchanged and this module's
    failures keep saying build-history-page.
    """
    return _measure_metric(REPO_ROOT, inst, spec, fail)
...
.tile-n {
  /* The heading weight comes from the token, not from a literal. These two
     rules asked for 700 on a face set that stops at 600, so the browser has
     always drawn them at 600 -- the same weight this page's own h1 asks for
     through --font-heading-weight, and the same weight the sibling root
     pages use. Measured in Chromium: at 700 the rendered advance width is
     identical to 600, so this changes the CSS and not the page. */
  font: var(--font-heading-weight) 28px/1.1 var(--font-heading);
  font-variant-numeric: tabular-nums;
}
.tile-l { font-size: 13px; color: var(--ink-2); margin-top: 4px; }
.tile-s { font-size: 12px; color: var(--muted); margin-top: 4px; }
h2 {
  font: var(--font-heading-weight) 20px/1.3 var(--font-heading);
