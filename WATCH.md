...
| Same day as any boundary rebuild above | The district-name search index (`il/data/app/district-search.json`, "33rd ward") holds one point and extent per district of wards, police districts, school board, Congress, IL Senate, IL House, IL Supreme Court and Board of Review | Run `python3 scripts/build_district_search.py` (network + shapely) and commit the result with the rebuild. CI's `--check` fails on its own for the six layers whose boundaries ship in `il/data/app`, but it CANNOT see a redrawn ward or police map, which the app loads live — for those two this row is the only reminder. | ☐ |
