...
GEOMETRY_FILES = {
    ...
    "adair-county-outline.json": (1, 1),  # Adair County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Adair County which recorded gap affects them: the statewide directory gives it a board size Iowa law does not allow, so no supervisor is named there. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "floyd-county-outline.json": (1, 1),  # Floyd County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Floyd County which recorded gap affects them: the statewide directory gives it a board size Iowa law does not allow, so no supervisor is named there. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "humboldt-county-outline.json": (1, 1),  # Humboldt County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Humboldt County which recorded gap affects them: the statewide directory gives it a board size Iowa law does not allow, so no supervisor is named there. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "lucas-county-outline.json": (1, 1),  # Lucas County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Lucas County which recorded gap affects them: the statewide directory gives it a board size Iowa law does not allow, so no supervisor is named there. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "pottawattamie-county-outline.json": (1, 1),  # Pottawattamie County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Pottawattamie County which recorded gap affects them: the statewide directory gives it a board size Iowa law does not allow, so no supervisor is named there. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "tama-county-outline.json": (1, 1),  # Tama County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Tama County which recorded gap affects them: the statewide directory gives it a board size Iowa law does not allow, so no supervisor is named there. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "warren-county-outline.json": (1, 1),  # Warren County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Warren County which recorded gap affects them: the statewide directory and the state's district map disagree about its board size, so no supervisor is named there. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "wright-county-outline.json": (1, 1),  # Wright County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Wright County which recorded gap affects them: the statewide directory and the state's district map disagree about its board size, so no supervisor is named there. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "black-hawk-county-outline.json": (1, 1),  # Black Hawk County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Black Hawk County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "calhoun-county-outline.json": (1, 1),  # Calhoun County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Calhoun County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "cass-county-outline.json": (1, 1),  # Cass County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Cass County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "dickinson-county-outline.json": (1, 1),  # Dickinson County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Dickinson County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "guthrie-county-outline.json": (1, 1),  # Guthrie County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Guthrie County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "ida-county-outline.json": (1, 1),  # Ida County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Ida County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "kossuth-county-outline.json": (1, 1),  # Kossuth County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Kossuth County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "lee-county-outline.json": (1, 1),  # Lee County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Lee County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "montgomery-county-outline.json": (1, 1),  # Montgomery County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Montgomery County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "osceola-county-outline.json": (1, 1),  # Osceola County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Osceola County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "palo-alto-county-outline.json": (1, 1),  # Palo Alto County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Palo Alto County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "sioux-county-outline.json": (1, 1),  # Sioux County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Sioux County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "washington-county-outline.json": (1, 1),  # Washington County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Washington County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
    "worth-county-outline.json": (1, 1),  # Worth County gap-location geometry — NOT a dispatched county and not a coverage test. It ships so the Data gaps panel can tell a reader standing in Worth County which recorded gap affects them: the county elects its supervisors by district and its districts ship, but nothing published says which supervisor holds which district, so the card names the board and cannot place any of them. Without this file `appliesHere` 404s while the panel's `mappable` flag is true and the lede claims "Nothing recorded is missing where you clicked" inside a county a gap covers. Built by ia/scripts/build_ia_gap_outlines.py from TIGERweb at the same 25 m tolerance the coverage wash uses, verified by the Census's own internal point for every gap-named county landing inside its own outline and outside all the others. The county list is DERIVED from the guidebook's own gap records, so an outline cannot outlive the gap that needs it.
}
...
ROSTER_FILES = {
    ...
    "ia-city-officials.json": 4,  # The elected officials of the FOUR Iowa cities whose own pages a machine can read AND whose robots.txt permits it — a mayor and five at-large council members apiece, 24 people with an e-mail each. A FIFTH, PALO, PUBLISHES A READABLE ROSTER AND IS NOT READ: cityofpalo.com names six crawlers, allows each everything but two admin paths, and ends `User-agent: * / Disallow: /`, so since 2026-09-05 the scraper consults robots.txt first and never requests the page. Palo stays in the scraper table and re-enters by itself if that file changes. NOT a statewide roster and never to be described as one: a sweep of all 532 Iowa cities that publish a website found 16 with a readable roster and 5 that cleared every check, against 407 cities publishing no site at all, so `ia-municipal-officeholders` stays open for the other 830 -- these four, the three cities that elect by ward, and the 102 named by their own COUNTY in ia-county-city-officials.json are the 109 that name anybody -- and the card says so on every one of them. All five are AT-LARGE, so they are roster rows on the City card and not a layer. Every address is re-tested each build against the name actually shipped by the same rule build_ia_county_officers.py applies — the officeholder's own name in the local part, or an office mailbox — because six of the 30 sit on consumer, provider or business domains that these small cities publish as official contact. Built by ia/scripts/ia_city_officials_scraper.py + build_ia_city_officials.py, refreshed weekly by update-ia-city-officials-roster.yml.
    "ia-county-city-officials.json": 96,  # The mayors, clerks and council members TEN IOWA COUNTIES publish for the 102 cities inside them — 738 officials, every one of the 102 naming a mayor, 89 naming a clerk, and 109 carrying the ward or at-large seat their own county states, which no other Iowa source pairs with a name at all. THE 738 ARE NOT ALL COUNCIL: 507 council members, 102 mayors and 90 clerk rows across 89 cities, plus 39 who are none of those — 12 city administrators, 10 city attorneys, 5 park commissioners, 5 park board, 5 library trustees, a city manager and an administrative secretary — shipped under the role their own county publishes, which is why the card's block is headed CITY OFFICIALS rather than claiming an election. TWO MARKUP FACTS ARE PER-COUNTY AND NOT UNIVERSAL, and both were first written here with JASPER ON THE WRONG SIDE OF EACH: SEVEN of the thirteen put the clerk's and mayor's role in a <b> inside the name's own div (Adams, Boone, Crawford, Iowa, Jasper, Marion, Winnebago) while the other SIX give every role a positionTitle heading — and the <b> is an ADDITION rather than an alternative, since all seven still take City Council from a heading, so what it carries is the NON-COUNCIL roles, Jasper's four being the widest vocabulary of the thirteen and the sole source of all ten City Attorney rows; and the mailto href is EMPTY in Boone, Crawford and Jasper (110 hrefs, none populated), POPULATED in Cerro Gordo, Shelby and Marion, and absent in the other SEVEN, ADAMS AMONG THEM — the scraper reads the link TEXT and the role structurally, so it needs neither to be universal. 29 OF THE 116 PHONES ARE THE CITY OFFICE'S OWN NUMBER RATHER THAN A DIRECT LINE, and each carries `phoneIsOffice` so the card says so instead of letting the row read as a personal line — every one a City Clerk, City Administrator or Library, never a mayor and never a council member, because the counties publish office contact for office staff and personal contact for elected members. The comparand is the city office number on that official's OWN record, never the Iowa League's per-city column: the two can name different numbers for one town, measured 2026-09-18 on Beaver, whose county prints (515) 386-2536 as the city's while its clerk's own row prints a different number and a personal address. Dropping the number instead would be the opposite answer to the one office_mailbox() already gives for e-mail, which keeps an office mailbox on the officeholder it reaches. This is the route `ia-municipal-officeholders`'s own blocker had recorded as NOT YET PROBED: the county auditors, Iowa's statutory commissioners of elections under Iowa Code 47.2. Thirteen counties publish such a page and three are REFUSED by a dated gate rather than by a hand list — Sac, Shelby and Winnebago still publish terms that ended in January 2026, and all three show their four-year seats split across 2025 and 2027 where the other ten straddle 2027 and 2029 (ADAMS straddles 2028 and 2030, later than any of them), the signature of a page last maintained after the November 2023 city election — so a county that updates its page ships on the next weekly run with nothing to edit here, and one that goes stale drops out the same way. NOT a statewide roster: 830 of Iowa's 939 cities still name nobody and the City card says so on every one of them. Built by ia/scripts/ia_county_city_officials_scraper.py + build_ia_county_city_officials.py, refreshed weekly by update-ia-county-city-officials-roster.yml.
    "ia-county-board-chairs.json": 33,  # Which supervisor chairs each county board, from two surfaces each county publishes itself: its board-of-supervisors page and its own board minutes. NO LIVE COUNT IS STATED HERE, because this one moves every week and a number written into a sentence is exactly what goes stale: the min_keys beside this entry is the page route's FLOOR, the shipped file's own total is printed by build_ia_county_chair.py on every run, and the reader-facing figure lives in one place only, the `ia-board-chair` gap record. CORRECTED 2026-09-21: this note said 43 of Iowa's 99 counties as of 2026-09-12, 38 from its board-of-supervisors page and 5 from its own board minutes, and was wrong twice over. 43 was a superseded total, since the file holds 38; and that 38 was the same total misread as the PAGE count, where the split ia/WATCH.md measured on 2026-09-18 is 33 from pages and 5 from minutes. The gap record and WATCH.md were both corrected in #1034 on 2026-09-19 and this note was missed, which is why it was the last place in the tree carrying 43. No statewide publisher records it: ISAC's directory, which is this app's source for the supervisors themselves, carries no chair field at all, and the chair is chosen by the board's own vote each January, so a one-time answer from 99 offices would be stale within the year — which is why this is a weekly re-scrape and not an ask. The `ia-board-chair` gap record had closed this as impossible on the ground that nothing could separate a real name from furniture like `Contact Information` or `Term Expires`; the separator was in the repo already, because a name that is not on that county's own supervisor roster is not a chair. Roster membership alone is NOT enough and an earlier attempt shipped a vice-chairman on it, so the pairing is structural: the role and the name must share the smallest DOM element that holds no OTHER supervisor, a role standing BEFORE a name is refused when a person-shaped phrase stands before it, and the two must sit within 30 characters (widest true pairing measured 28, tightest false one 84). Two further refusals came out of review: a chair QUALIFIED by another body is refused on a window cut backwards from the parent, and a pairing whose own term dates have expired is refused — the latter is why 35 shipped rather than 36 when that was measured, Mahaska's page dating itself two board terms ago — AND MAHASKA IS BACK, from its MINUTES: its board page is still stale and still refused, while its minutes are current and dated-filename, which is the whole argument for reading two surfaces rather than one. Boards-and-commissions pages are excluded outright — a supervisor's chairmanship of some other body is not evidence about the county board, which is how Des Moines County produced a wrong answer that all three structural tests passed. Of the 35 the page route shipped then, 19 were checkable against the county's own minutes and 19 agreed, none disagreed — which is what the MINUTES route was then built on. It reads the two most recent board minutes of counties the page route missed and RESOLVES six, requiring both documents to name the same chair — but ships FIVE (Cerro Gordo, Clinton, Dallas, Des Moines, Mahaska). JOHNSON RESOLVES AND DOES NOT SHIP: its minutes live on a Granicus portal whose robots.txt refuses `districtry` on every path, so the scraper asks before it fetches and never requests the page. The entry stays in the table, so the county re-enters by itself if that file changes. Its own trap is the flattened signature block: Des Moines County's `Shane McCampbell, Chair` / `Tom Broeker, Vice Chair` becomes one line in which the role has a name on BOTH sides, so an occurrence that resolves to two different members is refused and the attendance line settles it. Story County is refused on vocabulary rather than reach — three consecutive packets say `with Murken presiding` and never `chair`, and who gavelled a meeting is not who holds the office. The two routes are FLOORED SEPARATELY, because four counties is a tenth of this file and could collapse entirely behind the page route's healthy number; where the two disagree about a county the county is dropped rather than resolved by preferring a route. The card joins BY NAME so a chair who leaves the board stops rendering rather than sliding onto a successor. THE BOARD-PAGE SCRAPE GOT ITS OWN ROBOTS GATE ON 2026-09-06, which is why 34 ship where 35 did: Cherokee's site redirects /robots.txt to the Revize CMS, whose file ends `User-agent: * / Disallow: /`, so its page is no longer requested and its chair no longer ships. Hamilton is refused by the same vendor file and was never shipping. Both entries stay in the directory, so either re-enters by itself if that file changes. A COUNTY THAT GOES QUIET IS NOW REPORTED RATHER THAN SILENTLY DROPPED, and a narrow subset are carried: the builder reads the shipped file as an input and keeps a record only when this run's verdict was `unreachable` — the one verdict meaning we could not ASK — never on a robots refusal, never past 60 days from the record's own `confirmedOn`, never across a 1 January board reorganisation, and never for a name that has left the county's supervisor roster. A carried county counts toward neither route's floor. This exists because the 2026-09-08 run dropped Clayton and its log could not say why: the scrape printed a line only for a county it RESOLVED, so a county that left the file was visible only as a summary count that moved. Clayton's page names Ray Peterson as chairperson to this project's own client and did on the day it was dropped; the county sits behind Cloudflare, which answers one caller and not another. Both scrapers now print every county's verdict. Built by ia/scripts/ia_county_chair_scraper.py + ia_county_minutes_chair_scraper.py + build_ia_county_chair.py, refreshed weekly by update-ia-county-chair-roster.yml.
}
...
DYNAMIC_REFERENCE = frozenset({
    ...
    "adair-county-outline.json",
    "floyd-county-outline.json",
    "humboldt-county-outline.json",
    "lucas-county-outline.json",
    "pottawattamie-county-outline.json",
    "tama-county-outline.json",
    "warren-county-outline.json",
    "wright-county-outline.json",
    "black-hawk-county-outline.json",
    "calhoun-county-outline.json",
    "cass-county-outline.json",
    "dickinson-county-outline.json",
    "guthrie-county-outline.json",
    "ida-county-outline.json",
    "kossuth-county-outline.json",
    "lee-county-outline.json",
    "montgomery-county-outline.json",
    "osceola-county-outline.json",
    "palo-alto-county-outline.json",
    "sioux-county-outline.json",
    "washington-county-outline.json",
    "worth-county-outline.json",
})
