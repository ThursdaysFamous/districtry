...
| **New York** | [districtry.com/ny/](https://districtry.com/ny/) | Counties, towns and villages, school and judicial districts across the state, plus the city's boroughs, Council and community districts, NYPD precincts and school zones |
...
| | Elected School Board District | ERSB district ("6b"), elected board member |
...
| [willcountyillinois.gov](https://willcountyillinois.gov), [kanecountyil.gov](https://www2.kanecountyil.gov/pages/countyboard/boardMembers.aspx), [dupagecounty.gov](https://www.dupagecounty.gov) (scraped weekly by CI); [kendallcountyil.gov](https://www.kendallcountyil.gov/county-board/board-members), [mchenrycountyil.gov](https://www.mchenrycountyil.gov/departments/county-board/meet-your-county-board-members) (also scraped weekly by CI, since 2026-09-03 — these two need a stdlib HTTP client sending browser client hints, which their Akamai edge accepts where `requests` is refused) | Will / Kane / DuPage / Kendall / McHenry County Board rosters |
...
| ERSB shapefile | Elected School Board districts |
...
- **Static merge gate** (`scripts/validate_index.py`, run once per instance): the inline script passes `node --check`, every declared layer id is still registered (40 for Illinois), no dataset is embedded inline, every `data/app/*.json` file is present and shape-checked, and the sources page covers every registered layer.
