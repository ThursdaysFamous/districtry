...
| Kendall | 17093 | dispatch | districted | `county-board`, `county-precinct`, `fire-district`, `library-district`, `park-district` | 3 — `aurora-council-contact` (blocked); `blocked-crawlers` (blocked); `plano-ward-officials` (no-source) |
...
| McHenry | 17111 | dispatch | districted | `county-board`, `county-precinct`, `fire-district`, `judicial-subcircuit`, `library-district` | 3 — `blocked-crawlers` (blocked); `mchenry-park-district` (no-source); `mchenry-ward-cities` (blocked) |
